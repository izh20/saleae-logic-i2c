...
const App: React.FC = () => {
  ...
  const handleStopRecording = async () => {
    try {
      ...
      setRecordingMessage(filePath ? `Saved to ${filePath}` : 'Save cancelled');
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      setRecordingMessage(`Save failed: ${message}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ fontSize: 16, fontWeight: 600, margin: 0, whiteSpace: 'nowrap', flexShrink: 0 }}>
          Touchpad Tracker
        </h1>
        {appVersion && (
          <span style={{ fontSize: 11, color: '#858585', whiteSpace: 'nowrap' }}>
            v{appVersion}
          </span>
        )}
        <div
        ...
        />
        <span style={{ fontSize: 12, color: '#858585', whiteSpace: 'nowrap', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexShrink: 0 }}>
          ...
          ] satisfies Array<{ key: ViewMode; label: string }>).map(tab => (
            <button
              key={tab.key}
              onClick={() => {
        ...
        {/* Resolution config */}
        <div style={{ flexBasis: '100%', display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'flex-end' }}>
      ...
      {recordingMessage && (
        <div
          role="status"
          onClick={() => setRecordingMessage(null)}
          ...
          style={{
            ...
            borderRadius: 4,
            background: '#2d2d30',
            color: recordingMessage.startsWith('Save failed') ? '#f48771' : '#d4d4d4',
