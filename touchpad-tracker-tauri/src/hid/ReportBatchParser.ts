...
export function extractTouchFrames(frames: ParsedReportFrame[]): TouchFrame[] {
  ...
  const valueOf = (frameFields: ParsedReportFrame['fields'], ...names: string[]): number | undefined => {
    for (const name of names) {
      const field = frameFields[name];
  ...
  for (const frame of frames) {
    ...
    const scanTime = valueOf(f, 'Scan Time', 'scanTime') ?? 0;
    const contactCount = valueOf(f, 'Contact Count', 'contactCount') ?? 1;
    const button = valueOf(f, 'Button', 'button') ?? 0;

    const contacts: TouchContact[] = [];
    for (let c = 0; c < contactCount; c++) {
      const suffix = c > 0 ? ` ${c + 1}` : '';
      contacts.push({
        contactId: valueOf(f, `Contact Identifier${suffix}`) ?? c,
        x: valueOf(f, `X${suffix}`) ?? 0,
        y: valueOf(f, `Y${suffix}`) ?? 0,
        pressure: valueOf(f, `Tip Pressure${suffix}`) ?? 0,
        tipSwitch: valueOf(f, `Tip Switch${suffix}`) ?? 0,
        touchValid: valueOf(f, `Touch Valid${suffix}`, `In Range${suffix}`) ?? 0,
        inRange: valueOf(f, `In Range${suffix}`) ?? 0,
        width: valueOf(f, `Width${suffix}`),
        height: valueOf(f, `Height${suffix}`),
      });
    }
